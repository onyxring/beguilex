/* bgl-debug-v2 */
Quixe=function(){var self={};
/* bgl-debug injection: expose VM state to the WebView JS layer */
window._bglGetVmState=function(){
  ...
  if(typeof stack!=='undefined'&&stack){
    for(var _i=0;_i<stack.length;_i++){
      ...
      var _locs={};
      if(_frm.localsindex){
        for(var _j=0;_j<_frm.localsindex.length;_j++){
          var _p=_frm.localsindex[_j].pos;
  ...
  var _globals={};
  if(window._bglTrackedGlobalAddrs&&typeof Mem4!=='undefined'&&typeof memmap!=='undefined'&&memmap){
    var _addrs=window._bglTrackedGlobalAddrs;
    for(var _k=0;_k<_addrs.length;_k++){
      var _ga=_addrs[_k];
      _globals[_ga]=Mem4(_ga);
    }
  }
  return{frames:_frames,globals:_globals};
};
/* bgl-debug: read a single object property value from the live VM.
   obj    = object address; propId = compile-time property number.
   Returns the property value, an array for multi-word props, or null.
   Uses a direct property table scan:
     obj+16 = proptab ptr (GOBJFIELD_PROPTAB=4, offset 4*4=16).
     Table: 4-byte entry count, then n*10-byte entries:
       [0-1] propId  [2-3] len  [4-7] data-ptr  [8-9] flags
     data-ptr points to len*4 bytes of actual property data. */
window._bglReadProp=function(obj,propId){
  try{
    var ptab=Mem4(obj+16);
    if(!ptab)return null;
    var n=Mem4(ptab);
    for(var i=0;i<n;i++){
      var e=ptab+4+i*10;
      if(Mem2(e)!==propId)continue;
      var len=Mem2(e+2);
      var ptr=Mem4(e+4);          /* pointer to data block */
      if(len===0||!ptr)return null;
      if(len===1)return Mem4(ptr);/* single-word: dereference data pointer */
      var arr=[];for(var j=0;j<len;j++)arr.push(Mem4(ptr+j*4));
      return arr;
    }
    return null;
...
window._bglSetProp=function(obj,propId,val){
  try{
    ...
    for(var i=0;i<n;i++){
      ...
      var len=Mem2(e+2);
      var ptr=Mem4(e+4);
      if(len!==1||!ptr)return false; /* multi-word or no data: not writable */
      MemW4(ptr,val>>>0);
      return true;
    }
    return false;
...
/* bgl-debug: write a 32-bit value to a global variable's address in VM memory.
   Must only be called while the VM is paused. */
window._bglSetGlobal=function(addr,val){
  if(typeof MemW4!=='undefined'&&typeof memmap!=='undefined'&&memmap){
    MemW4(addr,val>>>0);return true;
  }return false;
};
/* bgl-debug: write a value to a local variable in a specific stack frame.
   stackIdx = index into the Quixe stack array (0=outermost, length-1=innermost).
   offset   = frame-offset from the .dbg file. */
window._bglSetLocal=function(stackIdx,offset,val){
  if(typeof stack!=='undefined'&&stack&&stack[stackIdx]){
    stack[stackIdx].locals[offset]=val|0;return true;
  }return false;
};
/* bgl-debug: read the 7 Glulx attribute bytes for an object.
   In I6/Glulx, the object record starts with a 1-byte type marker (0x70)
   followed by 7 attribute bytes.  Returns [byte0..byte6] or null. */
window._bglReadAttrs=function(obj){
  try{
    if(!obj)return null;
    var b=[];for(var i=0;i<7;i++)b.push(Mem1(obj+1+i));
    return b;
  }catch(_e){return null;}
};
/* bgl-debug: read the text of a Glulx dictionary word.
   Dictionary entries: 1 flag byte, then null-terminated word text.
   Returns the word text string, or null. */
window._bglDecodeDictWord=function(addr){
  try{
    if(!addr)return null;
    /* skip the leading flag byte */
    var s='',i=addr+1;
    while(Mem1(i)!==0&&s.length<50){s+=String.fromCharCode(Mem1(i));i++;}
    return s||null;
  }catch(_e){return null;}
};
/* bgl-debug: decode a Glulx string at the given VM address.
   Inform 6/Glulx string type bytes:
     0xE0 = null-terminated C-string (byte per char)
     0xE1 = Huffman compressed (bit stream, low-bit-first)
     0xE2 = Unicode (null-terminated 4-byte code points)
   Returns the decoded string, or null if the address is invalid or unsupported. */
window._bglDecodeString=function(addr){
  try{
    if(!addr)return null;
    var type=Mem1(addr);
    if(type===0xE0){
      /* C-string: null-terminated bytes starting at addr+1 */
      var s='',i=addr+1;
      while(Mem1(i)!==0&&s.length<500){s+=String.fromCharCode(Mem1(i));i++;}
      return s||null;
    }
    if(type===0xE2){
      /* Unicode string: null-terminated 4-byte code points */
      var s='',i=addr+1;
      while(s.length<500){var cp=Mem4(i);if(!cp)break;try{s+=String.fromCodePoint(cp);}catch(_){s+='?';}i+=4;}
      return s||null;
    }
    if(type!==0xE1)return null;
    /* Huffman compressed string (0xE1).
       Decompression table root: Mem4(Mem4(28)+8).
       Node layout — byte 0 is node type:
         0x00 = branch: left child at Mem4(node+1), right child at Mem4(node+5)
         0x01 = end of string
         0x02 = single char: byte at node+1
         0x03 = Unicode char: Mem4(node+1)
       Bit stream starts at addr+1, low-order bit of each byte first. */
    var dtab=Mem4(28);
    var root=Mem4(dtab+8);
    var s='',bitPos=0,node=root,dataAddr=addr+1;
    while(s.length<500){
      var nt=Mem1(node);
      if(nt===0x01)break;
      if(nt===0x02){s+=String.fromCharCode(Mem1(node+1));node=root;}
      else if(nt===0x03){s+=String.fromCodePoint(Mem4(node+1));node=root;}
      else if(nt===0x00){
        /* Glulx bit stream: low-order bit of each byte is read first */
        var byteIdx=bitPos>>3,bit=(Mem1(dataAddr+byteIdx)>>(bitPos&7))&1;
        bitPos++;node=bit?Mem4(node+5):Mem4(node+1);
      }else break;
    }
    return s||null;
  }catch(_e){return null;}
};
function quixe_prepare(image,all_options){game_image=image;var ls=game_image.slice(0,64);var ix,val;for(ix=0;ix<ls.length;ix++){val=ls[ix].toString(16);if(val.length<2)
val="0"+val;ls[ix]=val;}
...
if((vmfunc.pathaddrs[cp]||(window._bglSeqPts&&window._bglSeqPts.has(cp)))&&!context.path_ends){context.code.push("self.pc="+cp+";");oputil_unload_offstate(context);context.code.push("return;");context.path_ends=true;}}
...
total_path_calls++;if(window._bglBP&&window._bglBP.has(self.pc)&&self.pc!==window._bglSkipPC){window._bglCurrentVmFunc=vmfunc;window._bglCurrentIosys=self.iosysmode;window._bglSkipPC=null;self.done_executing=true;window._bglBreakPausing=true;window._bglIsPaused=true;if(window._bglOnBreak)window._bglOnBreak(self.pc);break;}if(window._bglStepMode){var _bglStepPC=self.pc;var _bglStop=!window._bglStepStopAt||window._bglStepStopAt.has(_bglStepPC);var _bglSkip=window._bglStepSkipAddrs&&window._bglStepSkipAddrs.has(_bglStepPC);if(_bglStop&&!_bglSkip){window._bglCurrentVmFunc=vmfunc;window._bglCurrentIosys=self.iosysmode;window._bglStepMode=false;self.done_executing=true;window._bglBreakPausing=true;window._bglIsPaused=true;if(window._bglOnStep)window._bglOnStep(_bglStepPC);break;}}window._bglSkipPC=null;var res=path(self);if(res===self.VMStopped){self.done_executing=true;self.vm_stopped=true;}}
pathend=new Date().getTime();total_execution_time+=(pathend-pathstart)/1000.0;if(self.vm_stopped){Glk.glk_exit();}
if(window._bglLog)window._bglLog('[glulx] execute_loop exit: breakPausing='+!!window._bglBreakPausing+' vmStopped='+!!self.vm_stopped+' resumefuncop='+(self.resumefuncop||'null')+' pc=0x'+(self.pc||0).toString(16));
if(window._bglBreakPausing){window._bglBreakPausing=false;return;}
if(window._bglLog)window._bglLog('[glulx] calling Glk.update()');
Glk.update();
if(window._bglLog)window._bglLog('[glulx] Glk.update() returned');qlog("### done executing; path time = "+(pathend-pathstart)+" ms");}
